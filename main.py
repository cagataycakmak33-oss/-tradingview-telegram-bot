...
BASE_PERIOD = 26
ADX_PERIOD = 14
# TradingView Fib indikatöründeki:
# Foption = "1. Candles"
# FP = 100
FIB_LOOKBACK = 100
MAX_WORKERS = 4
...
# =========================================================
#
# Gönderdiğin TradingView indikatöründeki:
#
# Foption = "1. Candles"
# FP      = 100
#
# mantığı:
#
# Fhigh = highest(high, 100)
# Flow  = lowest(low, 100)
#
# FH = highestbars(high, 100)
# FL = lowestbars(low, 100)
#
# revfibs = FL > FH
#
# Fib_x(n):
...
#     Fhigh - (Fhigh - Flow) * n
#
# Burada Pine Script'teki barsback mantığı:
# en yeni mum = 0
# daha eski mum = daha büyük sayı
#
# Bu nedenle:
# FL > FH
# demek dip daha eski,
# tepe daha yeni demektir.
#
def fibonacci_seviyeleri(df):
    ...
    if fhigh <= flow:
        return None
    # TradingView highestbars / lowestbars mantığı
    # Son mum = 0
    # Geriye gittikçe sayı büyür.
    #
    # idxmax/idxmin bize DataFrame sırasındaki
    # konumu verir. Bunu Pine barsback'e çeviriyoruz.
    high_pos = (
    ...
    #
    # Reverse = false olduğu için:
    # dip daha eskiyse yükseliş Fib'i.
    revfibs = (
        low_bars_back
        > high_bars_back
    )
    ...
    seviyeler = {
        "0.000": float(fib_x(0.000)),
        "0.236": float(fib_x(0.236)),
        "0.382": float(fib_x(0.382)),
        "0.500": float(fib_x(0.500)),
        "0.618": float(fib_x(0.618)),
        "0.786": float(fib_x(0.786)),
        "1.000": float(fib_x(1.000))
    }
    return {
        "high": fhigh,
        "low": flow,
...
def fib_destek_direnc_bul(
    ...
    # -----------------------------------------------------
    # TradingView'deki 0-1.000 ana Fib seviyelerini
    # fiyat açısından değerlendiriyoruz.
    #
    # Fiyatın altında kalan en yakın seviye = S1
    # Fiyatın üstündeki seviyeler = K1-K6
    # -----------------------------------------------------
    destekler = []
    direncler = []
...
def analiz_et(symbol):
    try:
        ...
        # 4 - RSI14 50 seviyesini aşağıdan yukarı kesiyor
        rsi_50_cross = (
            onceki["RSI14"] <= 50
            and
            son["RSI14"] > 50
